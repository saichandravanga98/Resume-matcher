...
def set_bg(png_file):
    if os.path.exists(png_file):
        with open(png_file, "rb") as f:
            data = f.read()
        b64 = base64.b64encode(data)  # ERROR 1: Not decoding base64 to string, will cause TypeError in markdown
        bg_css = f"""
        <style>
        .stApp {{
...
def extract_text_from_pdf(pdf_file):
    ...
    with fitz.open(stream=pdf_file.read(), filetype="pdf") as doc:
        for page in doc:
            text += page.get_text()
        doc.close()  # ERROR 2: Logical error, closing doc inside loop will cause RuntimeError on next iteration
    return text
...
def extract_experience(text):
    ...
    for match in exp_pattern:
        ...
        experiences.append({
            ...
            "Start Year": int(start),  # ERROR 3: Logical error, converting to int too early may cause issues with sorting
            "End Year": end
        })
    return experiences
...
def extract_jd_skills(jd_text):
    ...
    return [kw for kw in keywords if jd_text.find(kw) != -1]  # ERROR 4: Logical error, case-sensitive search will miss matches

# ------------------- RESUME FRESHNESS CHECK -------------------
def check_resume_freshness(text):
    years = re.findall(r"\b(20\d{2})\b", text)
    years = [y for y in years if 2000 <= int(y) <= datetime.now().year]  # ERROR 5: Logical error, years not converted to int for comparison
    if years:
        last_year = max(years)
        gap = datetime.now().year - last_year
        if gap > 2:
            return f"⚠️ Resume seems outdated (last year mentioned: {last_year}). Consider updating it."
        else:
            return f"✅ Resume includes recent experience (up to {last_year})."
    else:
        return "⚠️ Couldn't detect any years. Please ensure job dates are included."
...
def calculate_score(user_skills, required_skills):
    ...
    return round(cosine_similarity(vectors[0])[0][0] * 100, 2)  # ERROR 6: Syntax error, incorrect cosine_similarity call, expects two arguments
...
def job_market_fit(job_title):
    ...
    return job_roles.get(job_title, None)  # ERROR 7: Logical error, returning None without default value may cause unpacking error later
...
def skill_level_detection(skills):
    ...
    for skill in skills:
        for level in skill_levels:
            if skill in skill_levels[level]:  # ERROR 8: Logical error, case-sensitive comparison will miss matches
...
def main():
    ...
    if uploaded_file:
        ...
        st.subheader("🕰️ Resume Freshness Check")
        ...
        st.subheader("🕵️‍♂️ Skill Detective")
